...
(async()=>{
  try{
    const cleanKey='prestamo_ya_cache_clean_v24';
    ...
    await loadScript('./credit-proposal.js?v=2','module');
    const installGuards=()=>{
    ...
    const installLocationModule=()=>{
      ...
      const ensureUI=()=>{
        ...
        if(!panel){panel=document.createElement('div');panel.id='locationMapPanel';address.parentElement.appendChild(panel)}
        let tools=document.getElementById('locationTools');
        if(!tools){tools=document.createElement('div');tools.id='locationTools';tools.className='locationTools';tools.innerHTML='<button type="button" class="btn" id="openMapBtn">🗺️ Abrir mapa</button><button type="button" class="btn" id="openWazeBtn">🚗 Abrir Waze</button>';address.parentElement.appendChild(tools);document.getElementById('openMapBtn').onclick=()=>openResolvedMap();document.getElementById('openWazeBtn').onclick=()=>openWaze()}
        return {box,panel,tools};
      };
      const esc=s=>String(s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
      const parseLocation=()=>{const raw=String(location.value||'').trim().replace(/[\[\]]/g,'');const m=raw.split(',').map(Number);return m.length===2&&Number.isFinite(m[0])&&Number.isFinite(m[1])&&Math.abs(m[0])<=90&&Math.abs(m[1])<=180?{lat:m[0],lon:m[1]}:null};
      const renderMap=(lat,lon,label)=>{const {panel}=ensureUI();const d=0.0035;const bbox=[lon-d,lat-d,lon+d,lat+d].join('%2C');panel.innerHTML='<iframe loading="lazy" src="https://www.openstreetmap.org/export/embed.html?bbox='+bbox+'&layer=mapnik&marker='+encodeURIComponent(lat)+','+encodeURIComponent(lon)+'" title="Mapa de ubicación del cliente"></iframe><div id="locationResolved">📍 '+esc(label||('Ubicación: '+lat.toFixed(6)+', '+lon.toFixed(6)))+'</div>';panel.style.display='block'};
      const renderResults=(items)=>{const {box}=ensureUI();results=items||[];if(!results.length){box.innerHTML='<div class="locEmpty">No encontré esa dirección. Prueba agregando distrito, provincia o referencia.</div>';box.style.display='block';return}box.innerHTML=results.slice(0,5).map((x,i)=>'<div class="locItem" data-i="'+i+'"><b>'+esc(x.display_name?.split(',').slice(0,2).join(', ')||'Ubicación')+'</b><br><span class="muted">'+esc(x.display_name||'')+'</span></div>').join('');box.style.display='block';box.querySelectorAll('.locItem').forEach(el=>el.addEventListener('click',()=>selectResult(Number(el.dataset.i))))};
      const selectResult=(i)=>{const x=results[i];if(!x)return;const lat=Number(x.lat),lon=Number(x.lon);if(!Number.isFinite(lat)||!Number.isFinite(lon))return;address.value=x.display_name||address.value;location.value='['+lat.toFixed(6)+','+lon.toFixed(6)+']';const {box}=ensureUI();box.style.display='none';renderMap(lat,lon,x.display_name);toast('Ubicación encontrada y guardada')};
      const search=async(force=false)=>{const q=address.value.trim();if(q.length<4){const {box}=ensureUI();box.style.display='none';return}if(!force&&q===lastQuery)return;lastQuery=q;const {box}=ensureUI();box.innerHTML='<div class="locEmpty">🔎 Buscando dirección...</div>';box.style.display='block';try{const url='https://nominatim.openstreetmap.org/search?format=jsonv2&limit=5&addressdetails=1&countrycodes=pe&accept-language=es&q='+encodeURIComponent(q);const r=await fetch(url,{headers:{'Accept':'application/json'}});if(!r.ok)throw new Error('HTTP '+r.status);const data=await r.json();renderResults(data);if(data.length===1)selectResult(0)}catch(e){console.warn('Geocodificación:',e);box.innerHTML='<div class="locEmpty">No se pudo consultar el mapa. Puedes abrir Waze con la dirección escrita.</div>';box.style.display='block'}};
      window.searchClientAddress=search;window.openResolvedMap=()=>{const p=parseLocation(),q=address.value.trim();if(p){window.open('https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(p.lat+','+p.lon),'_blank','noopener');return}if(q)window.open('https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(q),'_blank','noopener');else toast('Ingrese primero una dirección')};window.openWaze=()=>{const p=parseLocation(),q=address.value.trim();if(!p&&!q)return toast('Ingrese primero una dirección');const url=p?'https://waze.com/ul?ll='+encodeURIComponent(p.lat+','+p.lon)+'&navigate=yes':'https://waze.com/ul?q='+encodeURIComponent(q)+'&navigate=yes';window.open(url,'_blank','noopener')};window.getLocation=()=>search(true);
      address.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>search(false),1000)});address.addEventListener('blur',()=>{clearTimeout(timer);timer=setTimeout(()=>search(false),250)});location.addEventListener('change',()=>{const p=parseLocation();if(p)renderMap(p.lat,p.lon,address.value||'Ubicación del cliente')});
      const oldEdit=window.editClient;if(typeof oldEdit==='function'&&!window.__moviaEditWrapped){window.editClient=(...args)=>{const r=oldEdit(...args);setTimeout(()=>{const p=parseLocation();if(p)renderMap(p.lat,p.lon,address.value||'Ubicación del cliente')},100);return r};window.__moviaEditWrapped=true}
      const oldNew=window.newClient;if(typeof oldNew==='function'&&!window.__moviaNewWrapped){window.newClient=(...args)=>{const r=oldNew(...args);setTimeout(()=>{const {panel,box}=ensureUI();panel.style.display='none';box.style.display='none'},100);return r};window.__moviaNewWrapped=true}
    };
    installLocationModule();
    console.log('Préstamo Ya: integración Cloud + propuesta de crédito + ubicación/Waze cargada correctamente');
