...
(()=>{const fs=require('fs'),assert=require('assert');const a=fs.readFileSync('v2/app/durable-actions-v2.js','utf8'),cards=fs.readFileSync('v2/app/operational-cards-v2.js','utf8'),shell=fs.readFileSync('v2/app/operational-shell.html','utf8');assert.ok(a.includes('root.MiCarteraV2DurableActions={')&&a.includes('renewInterest,refinance'),'renewal runtime exports must exist');assert.ok(cards.includes("MiCarteraV2DurableActions?.renewInterest")&&cards.includes("MiCarteraV2DurableActions?.refinance"),'renewal buttons must resolve durable runtime');assert.ok(shell.indexOf('operational-cards-v2.js')<shell.indexOf('durable-actions-v2.js'),'durable overrides must load after cards');console.log('V2 renewal runtime exports: PASS')})();
