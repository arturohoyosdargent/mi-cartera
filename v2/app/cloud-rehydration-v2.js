...
async function pushLocalMissing(local,names,rows,auth){
  ...
  for(const name of names){
    ...
    for(const row of localRows(local,name)){
      ...
      try{await f.setDoc(f.doc(f.db,'orgs',ORG,name,id),data,{merge:true});seen.add(id);written++;}
...
async function rehydrate(){
  ...
  for(let i=0;i<names.length;i++){
    ...
    if(target==='cashMovements')next[target].push(...effective);else next[target]=effective;
  }
...
function clearOnLogout(e){
  ...
  if(e?.detail?.preserveLocal===true){
    const local=readLocal();root.localStorage.setItem(K,JSON.stringify({...local,session:null}));
    dispatch('mi-cartera-v2-sync',{ok:true,source:'logout-session-only'});
    root.MiCarteraV2Agenda?.render?.();root.MiCarteraV2DashboardParity?.render?.();root.MiCarteraV2AdminParity?.render?.();return;
  }
  const local=readLocal(),next={...local,clients:[],credits:[],payments:[],cashMovements:[],audit:[],session:null};
  root.localStorage.setItem(K,JSON.stringify(next));dispatch('mi-cartera-v2-sync',{ok:true,source:'logout-clear'});
  root.MiCarteraV2Agenda?.render?.();root.MiCarteraV2DashboardParity?.render?.();root.MiCarteraV2AdminParity?.render?.();
}
let running=false;
async function onAuth(e){clearOnLogout(e);if(!e?.detail?.ready||running)return;running=true;try{await rehydrate()}catch(error){dispatch('mi-cartera-v2-rehydrated',{ok:false,error:String(error?.code||error?.message||error)})}finally{running=false}}
root.addEventListener('v2-auth-cloud-state',onAuth);
root.MiCarteraV2CloudRehydration={rehydrate,pending,pushLocalMissing};
if(root.document){const kick=()=>{const current=root.MiCarteraV2AuthCloudGate?.state?.();if(current?.ready)onAuth({detail:current})};if(root.document.readyState==='loading')root.addEventListener('DOMContentLoaded',kick);else setTimeout(kick,0);}
})(window);
