...
async function share(c,amount,date){
  ...
  const hasPhone=Boolean(String(c?.phone||'').trim());if(hasPhone&&root.MiCarteraV2CustomerExperience?.whatsapp){try{root.MiCarteraV2CustomerExperience.whatsapp(c,msg);return 'whatsapp'}catch(e){console.warn('V2 direct WhatsApp share failed; continuing with fallback.',e)}}
