...
async function share(c,amount,date){
  ...
  if(blob&&!hasPhone){try{const file=new File([blob],`prestamo-ya-recordatorio-${String(c?.name||'cliente').replace(/[^a-z0-9]+/gi,'-').toLowerCase()}.png`,{type:'image/png'});if(navigator.canShare?.({files:[file]})){await navigator.share({title:'PRÉSTAMO YA · Recordatorio de pago',text:msg,files:[file]});return 'file-share'}}catch(e){if(e?.name==='AbortError')return 'cancelled';console.warn('V2 reminder image sharing failed; continuing with fallback.',e)}}if(hasPhone&&root.MiCarteraV2CustomerExperience?.whatsapp){try{root.MiCarteraV2CustomerExperience.whatsapp(c,msg);return 'whatsapp'}catch(e){console.warn('V2 direct WhatsApp share failed; continuing with fallback.',e)}}
