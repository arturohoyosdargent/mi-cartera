...
function text(c,amount,date){const first=String(c?.name||'').trim().split(/\s+/)[0]||'cliente';return `Hola ${first}, te recordamos que tienes un pago pendiente de ${money(amount)}${date?' con vencimiento el '+fmt(date):''}.\n\nPRÉSTAMO YA · Gracias por tu confianza.`}
async function share(c,amount,date,editedMessage){
  const msg=String(editedMessage||'').trim()||text(c,amount,date);let blob=null;
  ...
  if(blob){try{const file=new File([blob],`prestamo-ya-recordatorio-${String(c?.name||'cliente').replace(/[^a-z0-9]+/gi,'-').toLowerCase()}.png`,{type:'image/png'});if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({title:'PRÉSTAMO YA · Recordatorio de pago',text:msg,files:[file]});return 'file-share'}}catch(e){if(e?.name==='AbortError')return 'cancelled';console.warn('V2 reminder image sharing failed; continuing with allowed text fallback.',e)}}
