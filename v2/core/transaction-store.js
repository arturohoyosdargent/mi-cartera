...
function createStore(adapter){assert(adapter&&typeof adapter.runAtomic==='function','ATOMIC_ADAPTER_REQUIRED');
 async function execute(operation){assert(operation&&sid(operation.id),'OPERATION_ID_REQUIRED');assert(Array.isArray(operation.writes)&&operation.writes.length,'WRITES_REQUIRED');const fp=fingerprint(operation);return adapter.runAtomic(async tx=>{const opPath=`operations/${sid(operation.id)}`;const existing=await tx.get(opPath);if(existing&&!existing.deletedAt){assert(existing.fingerprint===fp,'OPERATION_ID_COLLISION');return {status:'ALREADY_APPLIED',operation:existing};}const now=new Date().toISOString();for(const w of operation.writes){assert(w&&sid(w.path),'WRITE_PATH_REQUIRED');const current=await tx.get(w.path),cv=Number(current?.version||0);if(w.expectedVersion!=null)assert(cv===Number(w.expectedVersion),'VERSION_CONFLICT');if(w.kind==='create')assert(!current||current.tombstone,'DOCUMENT_ALREADY_EXISTS');if(w.kind==='delete'){await tx.set(w.path,{...(current||{}),deletedAt:now,tombstone:true,version:cv+1,updatedAt:now});}else{const requested=w.data?.version;assert(requested==null||Number(requested)===cv+1,'INVALID_NEXT_VERSION');await tx.set(w.path,{...(w.data||{}),version:cv+1,updatedAt:now});}}
