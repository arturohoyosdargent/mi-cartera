  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      ...
      try{
        ...
        // 1) El document ID de Firestore es la única identidad de la ruta.
        ...
        for(const r of routes){const name=norm(r.name);if(!name)continue;(groups.get(name)||groups.set(name,[]).get(name)).push(r);}
        ...
        for(const [name,list] of groups){
          ...
          const data=scored.filter(x=>x.cl||x.cr||x.pay);
          if(data.length>1)continue;
          const canonical=data[0]||scored.slice().sort((a,b)=>Number(!!b.owner)-Number(!!a.owner)||b.score-a.score||a.rid.localeCompare(b.rid))[0];
          if(!canonical)continue;
          for(const item of scored)for(const a of item.a)aliasToCanonical.set(a,canonical.rid);
          if(canonical.owner)canonicalOwners.set(canonical.rid,uidOf(canonical.owner));
          console.log('Préstamo Ya V2: ruta canónica',name,canonical.rid,canonical.cl,canonical.cr,canonical.pay);
        }

        // 2) Mover cartera por ID/alias histórico, nunca por nombre.
        ...
        const freshCredits=(await read('credits')).docs.map(d=>({docId:d.id,...d.data()}));
        const creditRoute=new Map(freshCredits.map(c=>[String(c.id||c.docId),String(c.routeId||'')]));
        ...
        // 3) Asignar al cobrador la ruta canónica y limpiar routeIds antiguos.
        routes=(await read('routes')).docs.map(d=>({docId:d.id,...d.data()}));
        for(const u of collectors){
          const ids=new Set();
          for(const x of Array.isArray(u.routeIds)?u.routeIds:[])ids.add(aliasToCanonical.get(String(x))||String(x));
        ...
        // 4) Verificación fuerte: solo se elimina un duplicado si no quedan referencias.
        const verifyRoutes=(await read('routes')).docs.map(d=>({docId:d.id,...d.data()}));
        ...
        const removed=[];
        for(const [candidate,canonical] of aliasToCanonical){
          if(candidate===canonical||!verifyRoutes.some(r=>onRoute(r)===candidate))continue;
          const used=verifyClients.some(x=>String(x.routeId)===candidate)||verifyCredits.some(x=>String(x.routeId)===candidate)||verifyPayments.some(x=>String(x.routeId)===candidate);
          if(!used){await deleteDoc(doc(fs,`orgs/${orgId}/routes`,candidate));removed.push(candidate);}
        }

        const finalRoutes=(await read('routes')).docs.map(d=>({id:d.id,...d.data(),routeId:d.id}));
        if(window.db){
          window.db.routes=finalRoutes;
          (window.db.clients||[]).forEach(c=>{const next=aliasToCanonical.get(String(c.routeId||''));if(next)c.routeId=next;});
          (window.db.credits||[]).forEach(c=>{const next=aliasToCanonical.get(String(c.routeId||''));if(next)c.routeId=next;});
