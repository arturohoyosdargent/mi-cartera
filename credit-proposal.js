...
(()=>{
  ...
  function install(){
    ...
    if(typeof rawSave==='function'){
      ...
      window.saveCredit=()=>{
        ...
        setTimeout(()=>applyExtrasAfterSave(before),80);
      };
    }
  }
  function baseData(){
  ...
  function addExtra(){
    const d=prompt('Fecha del pago adicional (AAAA-MM-DD):',today());if(!d)return;
  ...
  function renderExtras(){
    ...
    const totalExtra=state.extras.reduce((s,x)=>s+x.amount,0);const total= b.total+totalExtra;
    if($('cTotal'))$('cTotal').textContent=money(total);
  }
  function proposal(){
    ...
    const total= b.total+state.extras.reduce((s,x)=>s+x.amount,0);
    return {b,schedule,total};
  }
  function preview(send){
    const p=proposal();if(!p)return toast('Complete capital, primera fecha y número de cuotas');
    const c=window.db?.clients?.find(x=>String(x.id)===String(window.selectedClient));
  ...
  function applyExtrasAfterSave(before){
    try{
      const arr=window.db?.credits||[];const cr=arr.length>before?arr[arr.length-1]:arr.find(x=>String(x.id)===String(window.selectedCredit));if(!cr||!state.extras.length)return;
      const extraTotal=state.extras.reduce((s,x)=>s+x.amount,0);cr.total=Number(cr.total||0)+extraTotal;cr.schedule=Array.isArray(cr.schedule)?cr.schedule:[];
      const maxN=cr.schedule.reduce((m,q)=>Math.max(m,Number(q.n||0)),0);state.extras.forEach((x,i)=>cr.schedule.push({n:maxN+i+1,date:x.date,amount:x.amount,paid:0,postponed:false,extra:true}));
      cr.maturity=cr.schedule.reduce((m,q)=>String(q.date)>String(m)?q.date:m,cr.maturity||'');
      if(typeof window.persist==='function')window.persist();if(typeof window.enqueueSync==='function')window.enqueueSync('CREDITO_MODIFICADO',{id:cr.id,extraPayments:state.extras});
      state.extras=[];renderExtras();
