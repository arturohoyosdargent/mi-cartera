  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      ...
      try{
        ...
        // La identidad de una ruta es SIEMPRE su ID de documento. El nombre solo se muestra.
        // La asignación se normaliza a Firebase UID, que además satisface las reglas Cloud.
        for(const r of routes){
          ...
          if(match){
            const uid=uidOf(match);
            await setDoc(doc(fs,`orgs/${orgId}/routes`,routeIdOf(r)),{orgId,routeId:routeIdOf(r),collectorUid:uid,collectorId:uid},{merge:true});
            r.collectorUid=uid;r.collectorId=uid;
          }
        }

        routes=(await read('routes')).docs.map(d=>({docId:d.id,...d.data()}));
        const routeGroups=new Map();
        for(const r of routes){const name=norm(r.name);if(!name)continue;const a=routeGroups.get(name)||[];a.push(r);routeGroups.set(name,a);}
        const userAssignedIds=new Map();
        for(const u of collectors){
          const ids=new Set((Array.isArray(u.routeIds)?u.routeIds:[]).map(String));
          for(const r of routes)if(String(r.collectorUid||r.collectorId||'')===uidOf(u))ids.add(routeIdOf(r));
          userAssignedIds.set(uidOf(u),ids);
        }

        // Ruta canónica: primero asignación explícita; si no existe, mayor cartera; nunca por nombre.
        const canonicalByOldId=new Map();
        for(const [name,list] of routeGroups){
          if(list.length<2)continue;
          const scored=list.map(r=>{
            const rid=routeIdOf(r);
            const assigned=collectors.some(u=>userAssignedIds.get(uidOf(u))?.has(rid));
            const c=clients.filter(x=>String(x.routeId)===rid).length;
            const cr=credits.filter(x=>String(x.routeId)===rid).length;
            const p=payments.filter(x=>String(x.routeId)===rid).length;
            return {rid,assigned,score:c*100000+cr*1000+p};
          });
          const pool=scored.filter(x=>x.assigned).length?scored.filter(x=>x.assigned):scored;
          pool.sort((a,b)=>Number(b.assigned)-Number(a.assigned)||b.score-a.score||a.rid.localeCompare(b.rid));
        ...
        // Mover relaciones por ID estable.
        ...
        const freshCredits=(await read('credits')).docs.map(d=>d.data());
        const creditRoute=new Map(freshCredits.map(c=>[String(c.id),String(c.routeId??'')]));
        ...
        // routeIds del cobrador contienen únicamente IDs de rutas existentes y canónicas.
        routes=(await read('routes')).docs.map(d=>({docId:d.id,...d.data()}));
        for(const u of collectors){
          const ids=new Set();
          ...
          await setDoc(doc(fs,'users',u.docId),{routeIds:valid},{merge:true});
        }

        // Borrar duplicados solo después de comprobar que ninguna relación los utiliza.
        const verifyClients=(await read('clients')).docs.map(d=>d.data());
        const verifyCredits=(await read('credits')).docs.map(d=>d.data());
        const verifyPayments=(await read('payments')).docs.map(d=>d.data());
        for(const [oldId] of canonicalByOldId){
          const used=verifyClients.some(x=>String(x.routeId)===oldId)||verifyCredits.some(x=>String(x.routeId)===oldId)||verifyPayments.some(x=>String(x.routeId)===oldId);
          if(!used)await deleteDoc(doc(fs,`orgs/${orgId}/routes`,oldId));
        }

        const finalRoutes=(await read('routes')).docs.map(d=>({id:d.id,...d.data()}));
        if(window.db){
          window.db.routes=finalRoutes;
          (window.db.clients||[]).forEach(c=>{const next=canonicalByOldId.get(String(c.routeId));if(next)c.routeId=next;});
          (window.db.credits||[]).forEach(c=>{const next=canonicalByOldId.get(String(c.routeId));if(next)c.routeId=next;});
          (window.db.payments||[]).forEach(p=>{const cr=(window.db.credits||[]).find(c=>String(c.id)===String(p.creditId));if(cr?.routeId)p.routeId=cr.routeId;});
        ...
        window.__prestamoYaRouteMigrationDone=false;
        console.error('Préstamo Ya: migración estructural de rutas:',e);
      }
    });
