...
(function(){
  ...
  const normalize=(source)=>{
    ...
    const normalized=[];
    for(const key of critical){if(!Array.isArray(db[key])){db[key]=[];normalized.push(key)}}
  ...
  let attempts=0;
  const duringStartup=()=>{attempts+=1;try{normalize()}catch(e){console.warn('Préstamo Ya · contrato de estado',e)}if(attempts<40)setTimeout(duringStartup,250)};
