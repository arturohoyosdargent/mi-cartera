...
// v4: usa window.db/window.persist explícitamente para funcionar también como ES module.
...
(()=>{
 ...
 const process=async()=>{
  const db=window.db;
  if(!navigator.onLine||!db)return {ok:false,processed:0,remaining:db?.syncQueue?.length||0};
  const cfg=window.MI_CARTERA_FIREBASE||{},cloud=window.MI_CARTERA_CLOUD||{};
  if(!cloud.cloudEnabled||!cfg.projectId)return {ok:false,processed:0,remaining:db.syncQueue?.length||0};
  const app=await wait(),auth=getAuth(app),fs=getFirestore(app),u=auth.currentUser;
  if(!u)return {ok:false,processed:0,remaining:db.syncQueue?.length||0};
  const orgId=cloud.orgId||'mi-cartera';let profile=null;
  try{const snap=await getDoc(doc(fs,'users',u.uid));profile=snap.exists()?snap.data():null}catch(e){console.warn('Cola: perfil',e)}
  if(!profile&&typeof window.currentUser==='function')profile=window.currentUser()||null;
  ...
  for(const item of queue.slice()){
   ...
   const col=collectionFor(item.type);if(!col){item.status='ERROR';item.error='Tipo no soportado';continue;}
   ...
   const manager=profile?.role==='admin'||profile?.role==='supervisor';const routeOk=Array.isArray(profile?.routeIds)&&merged?.routeId!=null&&profile.routeIds.map(String).includes(String(merged.routeId));
   if(!manager&&!routeOk){item.attempts=(item.attempts||0)+1;item.status='ERROR';item.error='Sin permiso para la ruta';continue;}
   if(id==null){item.attempts=(item.attempts||0)+1;item.status='ERROR';item.error='No se pudo determinar el ID';continue;}
   try{item.attempts=(item.attempts||0)+1;const ref=doc(fs,`orgs/${orgId}/${col}`,id);if(isDelete(item.type))await deleteDoc(ref);else await setDoc(ref,clean({...merged,id,orgId,userId:u.uid,updatedAt:new Date().toISOString()}),{merge:true});item.status='SINCRONIZADO';item.syncedAt=new Date().toISOString();delete item.error;processed++;}catch(e){item.status='ERROR';item.error=String(e?.code||e?.message||e)}
  }
  db.settings=db.settings||{};db.settings.lastOnline=new Date().toISOString();
  try{if(typeof window.persist==='function')window.persist();else localStorage.setItem('mi_cartera_pro_v21',JSON.stringify(db))}catch(e){console.warn('Persist cola',e)}
  try{window.updateSyncUI?.()}catch(_){}
  const remaining=queue.filter(x=>x.status==='PENDIENTE'||x.status==='ERROR').length;
  window.__prestamoYaQueueLast={processed,remaining,at:new Date().toISOString()};
  return {ok:true,processed,remaining};
 };
 window.processSyncQueue=()=>process().catch(e=>{console.error('Cola Cloud',e);return {ok:false,processed:0,remaining:window.db?.syncQueue?.length||0,error:String(e)}});
 let running=false;
 ...
 if(!window.__prestamoYaQueueV3){window.__prestamoYaQueueV3=true;window.addEventListener('online',()=>setTimeout(()=>window.syncQueueV3(),1200));setTimeout(()=>window.syncQueueV3(),1800);setInterval(()=>{if(navigator.onLine)window.syncQueueV3()},20000)}
})();
