...
import { getFirestore, doc, getDoc, getDocFromServer, setDoc } from 'https://www.gstatic.com/firebasejs/12.18.0/firebase-firestore.js';

(()=>{
 const VERSION='credit-transaction-repair-v1';
 window.__prestamoYaTransactionRepair=async function(){
  ...
  try{
   ...
   for(const item of targets){
    ...
    if(id==null){item.status='ERROR';item.error='ID_NO_DETERMINADO';out.errors++;out.details.push({id:item?.id,type:item?.type,error:item.error});continue;}
    try{
     item.status='ENVIANDO'; item.error=''; if(typeof window.saveDB==='function')await window.saveDB(); if(typeof window.updateSyncUI==='function')window.updateSyncUI();
     ...
     item.status='SINCRONIZADO';item.syncedAt=new Date().toISOString();item.error='';out.processed++;out.details.push({id:String(id),type:item.type,status:item.status});
     if(typeof window.saveDB==='function')await window.saveDB(); if(typeof window.updateSyncUI==='function')window.updateSyncUI();
    }catch(e){item.status='ERROR';item.error=e?.code||e?.message||'SYNC_ERROR';item.lastAttempt=new Date().toISOString();out.errors++;out.details.push({id:String(id),type:item?.type,status:item.status,error:item.error});if(typeof window.saveDB==='function')await window.saveDB();if(typeof window.updateSyncUI==='function')window.updateSyncUI();}
   }
